  ...
  try {
    ...
    const [bookingResult] = await db.query(
      `INSERT INTO bookings
       ...
       VALUES (?, ?, ?, ?, ?, ?)`,
      [booking_id, req.user.id, dbFlightId, travel_class, seat_number || null, Number(total_amount)]
    );

    const bookingRowId = bookingResult.insertId;

    // ✅ Step 3: Sanitize and Insert passenger
    let sanitizedDob = passenger.dob || null;
    if (sanitizedDob) {
      // Basic check: Ensure year is reasonable (MySQL DATE range is 1000-01-01 to 9999-12-31)
      const year = parseInt(sanitizedDob.split("-")[0]);
    ...
    // Return specific status if it's a database constraint/value error
    const status = err.code === 'ER_TRUNCATED_WRONG_VALUE' ? 400 : 500;
    res.status(status).json({ 
      message: status === 400 ? "Invalid data provided (check date format)" : "Server error", 
      error: err.message 
    });
  }
};
